...
database.ref('/tumor_types/' + QueryString.id).once('value').then(function(snapshot) {
  ...
  if (tumorType == null) {
	  ...
	  if (tumorProperties!=null){
		  ...
		  //Add html to page
			$('#tumor-form').html(tumorFormHTML[0]);
		}
		...
		//console.log(tumorFormHTML[1]);

		//get a new optionlists from data base
		database.ref('/input_choice_list').once('value').then(function(snapshot) {
			//Get inputLists information and amke it global by leaving off var
			var inputLists = snapshot.val();
			//generate included question list
			var listoutHTML = generateOptionListHTML(inputLists, tumorFormHTML[1]);
			// add the questions included to HTML
			$('#optional-lists-output').html(listoutHTML);
...
function generateTumorFormHTML(tumorProperties) {
		...
		for (i=0; i<tumorProperties.length; i++) {
	  	//Get property information
			...
			if (propertyOptionalState != "required"){
					count +=1;
					questionList.push(propertyOptionalState);
				//Add property title and description to web page
				tumorFormHTML += '<tr><td><strong>' + propertyOptionalState + '</strong> <span class="name">' + propertyName +  '</span><span class="description">' +propertyDescription +'</span></td><td class="form-elements">';
			...
			return [tumorFormHTML, questionList];
...
function generateOptionListHTML(inputLists, includedQuestions){
//	console.log(includedQuestions);
	outHTML = ""
	//get a list of the input option lists
	for (item in inputLists){
		outHTML += '<p><strong>'+item+': </strong>';
			for (i =0; i < inputLists[item].length; i++){
				//console.log(i);
			//	console.log(inputLists[item][i]);
				if (includedQuestions.indexOf(inputLists[item][i])>=0){
					outHTML += inputLists[item][i] +" ";
				};
			};
